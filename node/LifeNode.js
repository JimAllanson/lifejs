...
Life.Renderer = function (parameters) {
    ...
    this.pause = function(){
        if(_worker){
            _worker.postMessage({action:"pause"});
        }
    };

    this.setParameters = function(parameters){
        if(_worker){
            _worker.postMessage({action:"setParameters", parameters:parameters});
        }
    };

    this.save = function(){
        if(_worker){
            _worker.postMessage({action:"save"});
        }
    };

    this.load = function(){
        if(_worker){
            _worker.postMessage({action:"load", world:_this.saved});
        }
    };

    this.clone = function(){
        if(_worker){
            _worker.postMessage({action:"clone"});
        }
    };

    this.kill = function(){
        if(_worker){
            _worker.postMessage({action:"kill"});
        }
    };
